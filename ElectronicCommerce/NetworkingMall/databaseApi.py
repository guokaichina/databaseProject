...
def create_goods(seller_id, goods_name, goods_stock, goods_price, goods_type):
    # 在数据库goods表，创建一个新的商品,创建成功返回goodsID，创建失败返回-1
    # 暂时先返回goods_id
    try:
        seller = Seller.objects.get(sellerID = seller_id)
        goods = Goods.objects.create(goodsName = goods_name, goodsStock = goods_stock,goodsPrice = goods_price, goodsType = goods_type,sellerID = seller)
...
def customer_login_by_name(name, password):
    # 登陆成功返回id，否则返回0
    user = Customer.objects.filter(customerName = name)
    if user.exists() == False:
        print('登录失败，用户名不存在')
        return 0
    else:
        if user.get().password == password:
            return user.get().customerID
        else:
            print('登录失败，密码错误')
            return 0
...
def customer_login_by_mail_address(mail_address, password):
    # 登陆成功返回id，否则返回0
    user = Customer.objects.filter(mailAddress = mail_address)
    if user.exists() == False:
        print('登录失败，用户邮箱不存在')
        return 0
    else:
...
def seller_login_by_mail_address(mail_address, password):
    # 登陆成功返回id，否则返回0
    user = Seller.objects.filter(mailAddress = mail_address)
    if user.exists() == False:
        print('登录失败，用户邮箱不存在')
        return 0
    else:
...
def seller_login_by_name(name, password):
    # 登陆成功返回id，否则返回0
    user = Seller.objects.filter(sellerName = name)
    if user.exists() == False:
        print('登录失败，用户名不存在')
        return 0
    else:
